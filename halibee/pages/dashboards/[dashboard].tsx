...
export default function Dashboard() {
    ...
    const confirmProjectCompletion = async (text: string) => {
        var confirmed = false;
       await Swal.fire({
        title: 'Are you sure?',
        text: "You won't be able to revert this! By clicking yes you confirm that you have " + text,
        icon: 'warning',
    ...
    const updateCompletedProjectDatabase = (projectDetails, reference) => {
        ...
        if (snapshot.val().shipped === true && snapshot.val().received === true) {
            push(ref(database, projectDetails.freelancerUID + "/completedProjects/"), {
                ...snapshot.val(),
                completionTime: new Date().getTime()
            });
            
            push(ref(database, projectDetails.clientUID + "/completedProjects/"), {
                ...snapshot.val(),
                completionTime: new Date().getTime()
            });
        }
    ...
    const updateCompletedProject = (projectDetails) => {
        ...
        const freelancerConfirmationText = "sent " + clientName + " the completed project."
        const clientConfirmationText = "received the completed project from " + freelancerName +'.' 

        if (currentUser.uid === projectDetails.clientUID && confirmProjectCompletion(clientConfirmationText)) {
        ...
        else if (currentUser.uid === projectDetails.freelancerUID && confirmProjectCompletion(freelancerConfirmationText)) {
            update(ref(database, freelancerReference), {
                shipped: true,
            });
    ...
    useEffect(() => {
        ...
        onAuthStateChanged(auth, (user) => {
            if (user) {
                ...
                onValue(
                    ref(database, user.uid + "/hireRequests/"),
                    (snapshot) => {
                        if (snapshot.val()) {
                ...
                onValue(
                    ref(database, user.uid + "/clientProjects/"),
                    (snapshot) => {
                ...
                onValue(
                    ref(database, user.uid + "/myProjects/"),
                    (snapshot) => {
                        if (snapshot.val()) {
                            for (var p in snapshot.val()) {
                                                                            ...
                                                                            <div className="bg-white rounded-lg relative">
                                                                                ...
                                                                                <div className="relative flex flex-row items-center justify-between min-w-40 break-words w-full mb-8 ">
                                                                                    ...
                                                                                    <button
                                                                                        className="uppercase h-auto px-2 mt-3 ml-6 bg-primary dark:bg-darkMode text-secondary w-max text-center font-semibold rounded-lg"
                                                                                        onClick={() =>
                                                                                            updateCompletedProject(clientProject)
                                                                                        }
                                                                                    >
                                                                                        Completed
                                                                                    </button>
                                                                ...
                                                                {({ active }) => (
                                                                    <div className="bg-white rounded-lg relative">
                                                                        ...
                                                                        <div className="relative flex flex-row items-center justify-between min-w-40 break-words w-full mb-8 ">
                                                                            ...
                                                                            <button
                                                                                className="uppercase h-auto px-2 mt-3 ml-6 bg-primary dark:bg-darkMode text-secondary w-max text-center font-semibold rounded-lg"
                                                                                onClick={() => updateCompletedProject(myProject)}
                                                                            >
                                                                                Completed
                                                                            </button>
                                                                        </div>
